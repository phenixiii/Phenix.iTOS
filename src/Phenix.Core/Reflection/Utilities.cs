    ...
    public static class Utilities
    {
        ...
        public static IList<Type> GetExportedClassTypes(Assembly assembly, bool includeAbstract = false)
        {
            ...
            foreach (Type item in assembly.GetExportedTypes())
                ...
                {
                    LogHelper.Warning(ex, "{@AssemblyName}", assembly.GetName().FullName);
                }
        ...
        /// <returns>类</returns>
        public static Type LoadType<T>(T obj)
        {
            Type type = typeof(T);
            return type == typeof(object) ? obj.GetType() : type;
        }
            ...
            where T : class
        {
            ...
            if (memberInfo is PropertyInfo propertyInfo)
            {
                if ((propertyInfo.ReflectedType ?? propertyInfo.DeclaringType).IsInterface)
                    propertyInfo = FindPropertyInfo(entity.GetType(), propertyInfo.Name) ?? propertyInfo;
            ...
            if (memberInfo is FieldInfo fieldInfo)
            {
                if ((fieldInfo.ReflectedType ?? fieldInfo.DeclaringType).IsInterface)
                    fieldInfo = FindFieldInfo(entity.GetType(), fieldInfo.Name) ?? fieldInfo;
            ...
            where T : class
        {
            ...
            return result != null && (result.ReflectedType ?? result.DeclaringType).IsInterface
                ? FindMemberInfo(typeof(T), result.Name, throwIfNotFound) ?? result
                : result;
        }
