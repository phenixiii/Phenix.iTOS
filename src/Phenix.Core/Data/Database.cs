...
namespace Phenix.Core.Data
{
    /// <summary>
    /// 数据库入口
    /// 由ConfigFilePath指定Phenix.Core.Init.db的PH7_Database表提供入口配置
    /// 默认入口为Default
    /// </summary>
                                ...
                                while (reader.Read())
                                    ...
                                    catch (NpgsqlException ex)
                                    {
                                        if (ex.Message.IndexOf("42P07:", StringComparison.Ordinal) == 0 || //表已存在
                                            ...
                                            continue;
                                        LogHelper.Error(ex, "{@SQL}: {@Caption}", reader.GetString(0), reader.GetString(1));
                                    }
#endif
#if MsSQL
                        ...
                        foreach (KeyValuePair<string, TimedTaskInfo> kvp in _timedTasks)
                            ...
                            catch (Exception ex)
                            {
                                ...
                                if (exceptionCount == _timedTasks.Count)
                                    throw;
                                LogHelper.Error(ex,"{@Key}", kvp.Key);
                            }
